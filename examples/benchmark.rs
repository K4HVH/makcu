...
fn main() -> Result<()> {
    ...
    println!("  move_xy          (100x):  avg={:>6}us  min={:>6}us  max={:>6}us", avg, min, max);
    ...
    println!("  silent_move      (100x):  avg={:>6}us  min={:>6}us  max={:>6}us", avg, min, max);

    device.silent_move(-100, 0)?;

    // -- wheel --
    let (avg, min, max) = bench_n(50, || device.wheel(1))?;
    println!("  wheel             (50x):  avg={:>6}us  min={:>6}us  max={:>6}us", avg, min, max);
    ...
    println!("  button down+up    (50x):  avg={:>6}us  min={:>6}us  max={:>6}us  (2 cmds per iter)", avg, min, max);

    // -- button_up_force --
    let (avg, min, max) = bench_n(50, || device.button_up_force(Button::Left))?;
    println!("  button_up_force   (50x):  avg={:>6}us  min={:>6}us  max={:>6}us", avg, min, max);
    ...
    println!("  set_lock on+off   (50x):  avg={:>6}us  min={:>6}us  max={:>6}us  (2 cmds per iter)", avg, min, max);

    // -- button_state (query, not execute) --
    let (avg, min, max) = bench_n(50, || { device.button_state(Button::Left)?; Ok(()) })?;
    println!("  button_state      (50x):  avg={:>6}us  min={:>6}us  max={:>6}us", avg, min, max);

    // -- lock_state (query) --
    let (avg, min, max) = bench_n(50, || { device.lock_state(LockTarget::X)?; Ok(()) })?;
    println!("  lock_state        (50x):  avg={:>6}us  min={:>6}us  max={:>6}us", avg, min, max);

    // -- version (query) --
    let (avg, min, max) = bench_n(20, || { device.version()?; Ok(()) })?;
    println!("  version           (20x):  avg={:>6}us  min={:>6}us  max={:>6}us", avg, min, max);

    // -- send_raw --
    let (avg, min, max) = bench_n(50, || { device.send_raw(b"km.version()\r\n")?; Ok(()) })?;
    println!("  send_raw          (50x):  avg={:>6}us  min={:>6}us  max={:>6}us", avg, min, max);
    ...
    println!("  ff 100 moves (cpp-style): total={:>6}us  per_cmd={:>4}us  ({:.0} cmd/s)",
        elapsed.as_micros(), per_cmd, 101.0 / elapsed.as_secs_f64());
    ...
    #[cfg(feature = "batch")]
    {
        ...
        device
            ...
            .execute()?;
        let elapsed = start.elapsed();
        println!("  batch 10 native cmds:    {:>6}us total  ({:>4}us per cmd equivalent)",
            elapsed.as_micros(), elapsed.as_micros() / 10);
        ...
        batch.execute()?;
        let elapsed = start.elapsed();
        println!("  batch 50 move_xy cmds:   {:>6}us total  ({:>4}us per cmd equivalent)",
            elapsed.as_micros(), elapsed.as_micros() / 50);
    }
    ...
    #[cfg(feature = "extras")]
    {
        ...
        println!("  click(50ms hold):        {:>6}us elapsed  ({:>+6}us overhead vs {}ms expected)",
            elapsed.as_micros(), overhead_us, expected_ms);
        ...
        let elapsed = start.elapsed();
        let overhead_us = elapsed.as_micros() as i64 - (expected_ms as i64 * 1000);
        println!("  move_smooth(10x10ms):    {:>6}us elapsed  ({:>+6}us overhead vs {}ms expected)",
            elapsed.as_micros(), overhead_us, expected_ms);
        ...
        let expected_ms = steps as u64 * interval_ms;
        let start = Instant::now();
        device.drag(Button::Left, 30, 0, steps, Duration::from_millis(interval_ms))?;
        let elapsed = start.elapsed();
        let overhead_us = elapsed.as_micros() as i64 - (expected_ms as i64 * 1000);
        println!("  drag(5x10ms):            {:>6}us elapsed  ({:>+6}us overhead vs {}ms expected)",
            elapsed.as_micros(), overhead_us, expected_ms);
